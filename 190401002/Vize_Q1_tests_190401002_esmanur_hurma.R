...
# SPOTIFY_ID ve SPOTIFY_SECRET değerlerini environment variable'lardan çek
spotify_id <- Sys.getenv("SPOTIFY_ID")
spotify_secret <- Sys.getenv("SPOTIFY_SECRET")

# Spotify token al
token <- spotifyr::spotify_token(client_id = spotify_id, client_secret = spotify_secret)

# Daha sonra token'ı kullanarak diğer işlemleri gerçekleştir
